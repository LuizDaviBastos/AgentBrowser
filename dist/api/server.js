...
const jobRunner_1 = require("../agent/jobRunner");
const browserDriver_1 = require("../browser/browserDriver");
...
class ApiServer {
    ...
    sockets = new Set();
    server = null;
    constructor(config = (0, config_1.loadConfig)()) {
        this.config = config;
        this.store = new jobStore_1.FileJobStore(config.dataDir);
        this.driver = config.browserMcpCommand
            ? new mcpBrowserDriver_1.McpBrowserDriver(config.browserMcpCommand, config.browserMcpArgs)
            : new browserDriver_1.NullBrowserDriver();
    }
    async start() {
        await this.driver.ensureReady().catch(() => { });
        this.server = node_http_1.default.createServer((req, res) => void this.route(req, res));
    ...
    async route(req, res) {
        try {
            ...
            if (req.method === 'GET' && url.pathname.startsWith('/assets/'))
                return this.serveSpaAsset(res, url.pathname);
            if (req.method === 'GET' && url.pathname === '/favicon.ico')
                return this.serveSpaAsset(res, '/favicon.ico');
            if (req.method === 'GET' && url.pathname === '/health')
                return this.sendJson(res, 200, { ok: true, status: 'ready' });
            if (req.method === 'GET' && url.pathname === '/openapi.json')
                return this.sendJson(res, 200, openapi_1.openApiDocument);
            if (req.method === 'GET' && url.pathname === '/jobs')
                return this.sendJson(res, 200, { items: this.store.list() });
            if (req.method === 'POST' && url.pathname === '/jobs')
                return this.createJob(req, res, url);
            const jobIdMatch = url.pathname.match(/^\/jobs\/([^/]+)(?:\/(cancel|retry|wait|run|logs))?$/);
            if (jobIdMatch)
                return this.handleJobAction(req, res, url, jobIdMatch[1], jobIdMatch[2]);
    ...
    async handleJobAction(req, res, url, id, action) {
        const job = this.store.get(id);
        if (!job)
            return this.sendJson(res, 404, { error: 'Job not found' });
        ...
        if (req.method === 'PUT' && !action)
            return this.updateJob(req, res, id, job);
        if (req.method === 'DELETE' && !action) {
            if (job.status === 'running')
                return this.sendJson(res, 409, { error: 'Job is already running' });
    ...
    async updateJob(req, res, id, job) {
        if (job.status === 'running')
            return this.sendJson(res, 409, { error: 'Job is already running' });
        const body = await readJson(req);
    ...
    async drain() {
        if (this.running >= this.config.maxConcurrentJobs)
            return;
        ...
        try {
            const result = await (0, jobRunner_1.executeJob)(job, this.driver, controller.signal, (level, message, data) => this.log(job.id, level, message, data), { deepseekApiKey: this.config.deepseekApiKey });
            ...
            const latest = this.store.get(job.id) || job;
            latest.status = canceled ? 'timed_out' : (latest.status === 'canceled' ? 'canceled' : 'failed');
    ...
    async waitForJobResult(id, timeoutMs) {
        const job = this.store.get(id);
        if (!job)
            return { status: 404, body: { error: 'Job not found' } };
        if (isActive(job.status)) {
    ...
    validateJobRequest(body, base) {
        ...
        if (instruction.length > this.config.maxJobInstructionChars)
            return { error: 'instruction too long' };
        const inferredSchema = inferOutputSchema(payload.outputSchema ?? base?.outputSchema, payload.outputExample ?? base?.outputExample);
        if (!inferredSchema)
            return { error: 'outputSchema or outputExample is required' };
        return {
            ...
            requiredFields: Array.isArray(payload.requiredFields) ? payload.requiredFields.filter((x) => typeof x === 'string') : base?.requiredFields,
            ...
            metadata: payload.metadata && typeof payload.metadata === 'object' ? payload.metadata : base?.metadata,
    ...
    serveSpa(res) {
        ...
        if (indexPath)
            return this.sendStaticFile(res, indexPath, 'text/html; charset=utf-8', true);
        return this.sendHtml(res, 200, renderDashboard(this.config));
    }
    serveSpaAsset(res, urlPath) {
        const assetPath = this.resolveSpaAssetPath(urlPath);
        if (!assetPath)
            return this.sendHtml(res, 404, 'Not found');
        this.sendStaticFile(res, assetPath, this.contentTypeForPath(assetPath), false);
    }
    sendStaticFile(res, filePath, contentType, noStore) {
        try {
    ...
    getSpaRoot() {
        return node_path_1.default.resolve(process.cwd(), 'web', 'dist');
    }
    getSpaIndexPath() {
        const indexPath = node_path_1.default.join(this.getSpaRoot(), 'index.html');
        return node_fs_1.default.existsSync(indexPath) ? indexPath : null;
    }
    resolveSpaAssetPath(urlPath) {
        const root = this.getSpaRoot();
        const candidate = node_path_1.default.resolve(root, `.${urlPath}`);
...
function readBooleanQuery(raw, fallback) {
    if (raw == null)
        return fallback;
...
function appendExecution(job, execution) {
    if (!Array.isArray(job.executions))
        job.executions = [];
...
function inferOutputSchema(schemaOrExample, example) {
    if (schemaOrExample && typeof schemaOrExample === 'object' && looksLikeSchema(schemaOrExample)) {
        return schemaOrExample;
    }
